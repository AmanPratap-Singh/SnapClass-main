...
def teacher_dashboard():
    ...
    with tab1:
        type1 = type="primary" if st.session_state.current_teacher_tab == 'take_attendance' else 'tertiary'
        if st.button('Take Attendance',type=type1, width='stretch', icon="📸"):
            st.session_state.current_teacher_tab = 'take_attendance'
            st.rerun()

    with tab2:
        type2 = type="primary" if st.session_state.current_teacher_tab == 'Manage Subjects' else 'tertiary'
        if st.button('Manage Subjects', type=type2, width='stretch', icon="📚"):
            st.session_state.current_teacher_tab = 'manage_subjects'
            st.rerun()

    with tab3:
        type3 = type="primary" if st.session_state.current_teacher_tab == 'Attendance Records' else 'tertiary'
...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        for sub in subjects:
            ...
            def share_btn():
                if st.button(f"Share Code: {sub['name']}", key=f"share_{sub['subject_code']}", icon="🔗"):
